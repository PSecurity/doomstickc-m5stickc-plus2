/*
  ...
  Novidades v4.0:
    - 4 tipos de inimigos: Normal, Fast, Tank, Ranged
    - 8 inimigos por fase
    - 3 armas: Pistol, Blaster, Shotgun (pickup S)
    - Inimigo Ranged atira projéteis simulados
    - Score: kills + bônus de tempo
    - Modos de dificuldade: Easy / Normal / Hard (Power na intro)
    - 5 fases (mapas 20x16)
    - Sala secreta: parede W abre com Power

  Controles:
    - Tela inicial: A inicia, Power muda dificuldade
    - Botão A: andar para frente
    - Botão B curto: atirar
    - Botão B longo: trocar arma
    - Inclinar esquerda/direita: virar
    - Power click rápido: usar / abrir porta / parede falsa
    - Power segurado: correr
...
static constexpr float MAX_DEPTH      = 16.0f;
...
static void startLevelClear() {
  DoomStickCGameplay::finishLevelTimer(millis());
...
static void loadLevel(int level, bool keepStats) {
  ...
  if (!keepStats) {
    DoomStickCPlayer::resetStats(player, DoomStickCGameplay::difficultyStartAmmo());
  } else {
    DoomStickCPlayer::applyLevelBonus(
      player,
      DoomStickCGameplay::hpBonusForNextLevel(currentLevel - 1),
      DoomStickCGameplay::ammoBonusForNextLevel(currentLevel - 1)
    );
  }
  for (int i = 0; i < ENEMY_COUNT; i++) {
    DoomStickCEnemies::loadFromSpawn(enemies[i], DoomStickCMaps::LEVEL_ENEMIES[currentLevel][i]);
  }
  sprintUntilMs = 0; shootFlashUntilMs = 0;
  damageFlashUntilMs = 0; emptyAmmoFlashUntilMs = 0; pickupFlashUntilMs = 0;
  powerWasDown = false; powerHoldFired = false;
  fireButtonWasDown = false; fireButtonHoldFired = false;
  gameState = GAME_LEVEL_START;
  levelStartStartedMs = millis();
  DoomStickCGameplay::startLevelTimer(millis());
...
static void resetGame(bool startPlaying) {
  ...
  powerWasDown = false; powerHoldFired = false;
  fireButtonWasDown = false; fireButtonHoldFired = false;
  setStatus(startPlaying
    ? (String(DoomStickCUI::STATUS_LEVEL_PREFIX) + String(1))
    : String(DoomStickCUI::STATUS_READY), 1000);
}
...
static void handlePickupAtPlayer() {
  ...
  if (c == 'H') {
    ...
    DoomStickCWeapons::unlockShotgun(weaponState);
    player.ammo += 8;
    worldMap[my][mx] = '.';
    pickupFlashUntilMs = millis() + 220;
    playToneSafe(DoomStickCAudio::TONE_LEVEL + 180, 110);
    setStatus(DoomStickCUI::STATUS_SHOTGUN_PICKUP);
  } else if (c == 'E') {
    worldMap[my][mx] = '.';
    startLevelClear();
  }
...
static void useAhead() {
  ...
  if (tx >= 0 && tx < MAP_W && ty >= 0 && ty < MAP_H) {
    ...
    // Parede falsa W — sala secreta
    if (c == 'W') {
      worldMap[ty][tx] = '.';
      playToneSafe(DoomStickCAudio::TONE_PICKUP + 400, 120);
      setStatus(DoomStickCUI::STATUS_SECRET, 2000);
      return;
    }
  }
...
static void switchWeapon() {
  if (!weaponState.blasterUnlocked && !weaponState.shotgunUnlocked) {
...
static void shoot() {
  ...
  int toneFreq = DoomStickCAudio::TONE_SHOOT + DoomStickCWeapons::toneBoost(weaponState.current);
  if (toneFreq < 80) toneFreq = 80;
  playToneSafe(toneFreq, 35);

  bool isArea = DoomStickCWeapons::isAreaWeapon(weaponState.current);
  bool hitAny = false;
  ...
  for (int i = 0; i < ENEMY_COUNT; i++) {
    ...
    if (delta < DoomStickCWeapons::aimCone(weaponState.current) &&
        hasLineOfSight(player.x, player.y, enemies[i].x, enemies[i].y)) {
      bool died = DoomStickCEnemies::hitEnemy(enemies[i]);
      if (died) {
        DoomStickCGameplay::registerKill();
        ...
        setStatus(DoomStickCUI::STATUS_TANK_HIT);
      }
      hitAny = true;
      if (!isArea) break;  // Pistol/Blaster: apenas 1 inimigo
    }
  }
  if (!hitAny) setStatus(DoomStickCUI::STATUS_SHOT);
}
...
static void updateEnemies(float dt) {
  ...
  for (int i = 0; i < ENEMY_COUNT; i++) {
    ...
    EType type = enemies[i].type;
    float speed = DoomStickCGameplay::enemySpeedForLevel(currentLevel)
                * DoomStickCEnemies::speedMultiplier(type);
    int   dmg   = DoomStickCGameplay::enemyDamageForLevel(currentLevel)
                * DoomStickCEnemies::damageMultiplier(type);

    // ---- Ranged: para e atira de longe ----
    if (type == EType::Ranged) {
      if (dist < DoomStickCEnemies::RANGED_SHOOT_DIST &&
          hasLineOfSight(enemies[i].x, enemies[i].y, player.x, player.y)) {
        // Atira se cooldown passou
        if (DoomStickCEnemies::canRangedShoot(enemies[i], now)) {
          DoomStickCEnemies::markRangedShot(enemies[i], now);
          // Projétil simulado: dano direto se linha de visão livre
          if (DoomStickCEnemies::canDamage(enemies[i], now)) {
            ...
            playToneSafe(DoomStickCAudio::TONE_DAMAGE - 40, 80);
            setStatus(String("-") + String(dmg) + String(" Projétil"), 700);
            if (player.hp <= 0) {
              gameState = GAME_DEAD;
        ...
        // Ranged mantém distância mínima
        if (dist < DoomStickCEnemies::RANGED_STOP_DIST) continue;
      }
    }
    ...
    if (dist < ENEMY_ATTACK_DISTANCE) {
      if (DoomStickCEnemies::canDamage(enemies[i], now)) {
        ...
        setStatus(String("-") + String(dmg) + String(" Vida"), 700);
        if (player.hp <= 0) {
          gameState = GAME_DEAD;
          playToneSafe(DoomStickCAudio::TONE_DEAD, 250);
          setStatus(DoomStickCUI::STATUS_DEAD, 3000);
    ...
    // ---- Movimento ----
    float sightDist = (type == EType::Ranged) ? 9.0f : 7.5f;
    if (dist < sightDist && hasLineOfSight(enemies[i].x, enemies[i].y, player.x, player.y)) {
      float inv = 1.0f / max(0.01f, dist);
      float vx = dx * inv * speed * dt;
      float vy = dy * inv * speed * dt;
      float nx = enemies[i].x + vx;
      float ny = enemies[i].y + vy;
      if (canEnemyMoveTo(nx, enemies[i].y)) enemies[i].x = nx;
      if (canEnemyMoveTo(enemies[i].x, ny)) enemies[i].y = ny;
    }
  }
}
...
static void drawPeekSecurityLoadingScreen() {
  ...
  for (int y = 0; y < SCREEN_H; y += 6)  frame.drawFastHLine(0, y, SCREEN_W, rgb(5, 5, 14));
  for (int x = 0; x < SCREEN_W; x += 16) frame.drawFastVLine(x, 0, SCREEN_H, rgb(8, 5, 18));
  ...
  if      (step == 0) frame.print(DoomStickCUI::LOADING_BOOT_0);
  else if (step == 1) frame.print(DoomStickCUI::LOADING_BOOT_1);
  else if (step == 2) frame.print(DoomStickCUI::LOADING_BOOT_2);
  else                frame.print(DoomStickCUI::LOADING_BOOT_3);
  frame.setTextColor(green, BLACK);
...
static void drawIntroScreen() {
  ...
  frame.drawString(DoomStickCVersion::TITLE, SCREEN_W / 2, 22, 4);
  frame.setTextColor(WHITE, BLACK);
  frame.drawString(DoomStickCVersion::HARDWARE, SCREEN_W / 2, 48, 2);
  frame.setTextColor(rgb(120, 255, 180), BLACK);
  frame.drawString(DoomStickCVersion::INTRO_LABEL, SCREEN_W / 2, 65, 2);

  // Dificuldade selecionada
  using D = DoomStickCGameplay::Difficulty;
  uint16_t diffCol = (DoomStickCGameplay::currentDifficulty == D::Easy)  ? rgb(80, 255, 120) :
                     (DoomStickCGameplay::currentDifficulty == D::Hard)  ? rgb(255, 80, 80)  :
                                                                            rgb(255, 200, 60);
  frame.setTextColor(diffCol, BLACK);
  frame.drawString(
    String(DoomStickCUI::DIFF_PREFIX) +
    String(DoomStickCGameplay::DIFFICULTY_LABELS[static_cast<int>(DoomStickCGameplay::currentDifficulty)]),
    SCREEN_W / 2, 83, 2
  );

  uint32_t pulse = (millis() / 280) % 2;
  frame.setTextColor(pulse ? YELLOW : rgb(180, 180, 180), BLACK);
  frame.drawString(DoomStickCUI::INTRO_START, SCREEN_W / 2, 102, 2);
  frame.setTextColor(rgb(145, 90, 255), BLACK);
  frame.drawString(DoomStickCUI::INTRO_CONTROLS, SCREEN_W / 2, 118, 1);
  frame.setTextColor(rgb(100, 180, 255), BLACK);
  frame.drawString(DoomStickCUI::INTRO_DIFF_HINT, SCREEN_W / 2, 130, 1);
  frame.pushSprite(0, 0);
}
...
static void drawHud() {
  ...
  if      (player.hp <= 25) frame.setTextColor(RED, hudBg);
  else if (player.hp <= 55) frame.setTextColor(YELLOW, hudBg);
  else                      frame.setTextColor(rgb(120, 255, 180), hudBg);
  ...
  frame.setCursor(140, 5);
  frame.print(DoomStickCUI::HUD_ENEMIES);
  frame.print(enemiesAliveCount());

  frame.setTextColor(rgb(255, 220, 80), hudBg);
  frame.setCursor(163, 5);
  frame.print(DoomStickCUI::HUD_KILLS);
  frame.print(DoomStickCGameplay::score.totalKills);

  frame.setTextColor(DoomStickCWeapons::hudColor(M5.Display, weaponState.current), hudBg);
  frame.setCursor(183, 5);
...
static void drawWeapon() {
  ...
  using WId = DoomStickCWeapons::WeaponId;

  if (weaponState.current == WId::Blaster) {
    ...
    frame.fillCircle(cx + 15, baseY - 18, 3, edge);
  } else if (weaponState.current == WId::Shotgun) {
    // Cano duplo largo
    frame.fillRoundRect(cx - 36, baseY - 36, 72, 38, 6, dark);
    frame.drawRoundRect(cx - 36, baseY - 36, 72, 38, 6, edge);
    frame.fillRoundRect(cx - 24, baseY - 28, 48, 28, 5, metal);
    frame.drawFastVLine(cx - 10, baseY - 25, 20, rgb(40, 28, 10));
    frame.drawFastVLine(cx + 10, baseY - 25, 20, rgb(40, 28, 10));
    // Cano duplo
    frame.fillRect(cx - barrelW / 2, baseY - 54, barrelW / 2 - 1, barrelH, metal2);
    frame.fillRect(cx + 1, baseY - 54, barrelW / 2 - 1, barrelH, metal2);
    frame.drawRect(cx - barrelW / 2, baseY - 54, barrelW, barrelH, edge);
    // Coronha
    frame.fillRect(cx + 20, baseY - 30, 14, 20, rgb(90, 60, 20));
  } else {
    // Pistol
  ...
  if (millis() < shootFlashUntilMs) {
    ...
    if (weaponState.current == WId::Blaster) {
      ...
      // Flash duplo largo
      frame.fillTriangle(cx, baseY - 88, cx - 40, baseY - 50, cx + 40, baseY - 50, flashOuter);
      frame.fillTriangle(cx, baseY - 72, cx - 22, baseY - 54, cx + 22, baseY - 54, flashInner);
      frame.fillCircle(cx - 10, baseY - 56, 5, flashOuter);
      frame.fillCircle(cx + 10, baseY - 56, 5, flashOuter);
    } else {
...
static void drawMiniMap() {
  const int scale = 2;   // mapa 20x16 — escala menor para caber
  const int ox = 4;
  const int oy = 22;
  ...
  for (int y = 0; y < MAP_H; y++) {
    for (int x = 0; x < MAP_W; x++) {
      ...
      if (c == 'W') col = rgb(60, 55, 90);   // parede falsa — quase igual
  ...
  for (int i = 0; i < ENEMY_COUNT; i++) {
    ...
    switch (enemies[i].type) {
      case EType::Fast:   ec = rgb(255, 180, 30); break;
      case EType::Tank:   ec = rgb(80, 200, 80);  break;
      case EType::Ranged: ec = rgb(140, 80, 255); break;
      default:            ec = RED;               break;
    }
    frame.fillRect(ex - 1, ey - 1, 3, 3, ec);
  }

  int px = ox + (int)(player.x * scale);
  int py = oy + (int)(player.y * scale);
  frame.fillCircle(px, py, 2, rgb(90, 255, 160));
  frame.drawLine(px, py, px + (int)(cosf(player.a) * 4), py + (int)(sinf(player.a) * 4), rgb(90, 255, 160));
}
...
static void drawEnemies() {
  for (int i = 0; i < ENEMY_COUNT; i++) {
    ...
    if (fabsf(delta) > FOV * 0.56f) continue;
    int sx      = (int)((0.5f + delta / FOV) * SCREEN_W);
    int spriteH = DoomStickCSprites::enemySpriteHeight(dist, enemies[i].type);
    int spriteW = DoomStickCSprites::enemySpriteWidth(spriteH, enemies[i].type);
...
static void renderWorld() {
  frame.fillScreen(BLACK);
  DoomStickCRender::drawSkyAndFloor(frame, SCREEN_W, SCREEN_H, 19,
    rgb(20, 20, 40), rgb(28, 24, 23), rgb(45, 35, 55));
  ...
  for (int x = 0; x < SCREEN_W; x += RAY_COLUMN_STEP) {
    ...
    while (!hitWall && distanceToWall < MAX_DEPTH) {
      ...
      if (hitCell == '#' || hitCell == 'D' || hitCell == 'W') hitWall = true;
    }
    ...
    // Parede falsa W: cor levemente diferente para dar dica sutil
    if (hitCell == 'W') {
      wallCol = M5.Display.color565(
        static_cast<uint8_t>(shade * 0.38f + 8),
        static_cast<uint8_t>(shade * 0.32f + 4),
        static_cast<uint8_t>(shade * 0.95f)
      );
    }
    ...
    if (colIndex % 8 == 0)  frame.drawFastVLine(x, ceiling, floorY - ceiling, rgb(10, 10, 18));
    if (colIndex % 13 == 0) {
  ...
  if (millis() < pickupFlashUntilMs)    frame.drawRect(0, 0, SCREEN_W, SCREEN_H, rgb(120, 255, 180));
...
static void drawLevelClearScreen() {
  ...
  frame.drawString(DoomStickCVersion::STATUS_LABEL, SCREEN_W / 2, 18, 2);
  frame.setTextColor(rgb(120, 255, 180), BLACK);
  frame.drawString(DoomStickCUI::LEVEL_CLEAR_TITLE, SCREEN_W / 2, 46, 4);

  // Kills e tempo da fase
  uint32_t sec = DoomStickCGameplay::score.levelTimeMs / 1000;
  frame.setTextColor(rgb(255, 220, 80), BLACK);
  frame.drawString(
    String(DoomStickCUI::KILLS_PREFIX) + String(DoomStickCGameplay::score.levelKills) +
    String("  ") + String(DoomStickCUI::TIME_PREFIX) + String(sec) + String("s"),
    SCREEN_W / 2, 74, 2
  );

  frame.setTextColor(WHITE, BLACK);
  frame.drawString(String(DoomStickCUI::LEVEL_CLEAR_PREFIX) + String(currentLevel + 2), SCREEN_W / 2, 96, 2);
  frame.setTextColor(YELLOW, BLACK);
  frame.drawString(DoomStickCUI::LEVEL_CLEAR_BONUS, SCREEN_W / 2, 116, 2);
  frame.pushSprite(0, 0);
}
...
static void drawEndScreen(const char* title, const char* subtitle, uint16_t titleColor) {
  ...
  frame.setTextSize(1);
  frame.setTextColor(rgb(155, 90, 255), BLACK);
  frame.drawString(DoomStickCVersion::STATUS_LABEL, SCREEN_W / 2, 16, 2);
  frame.setTextColor(titleColor, BLACK);
  frame.drawString(title, SCREEN_W / 2, 46, 4);
  frame.setTextColor(WHITE, BLACK);
  frame.drawString(subtitle, SCREEN_W / 2, 76, 2);

  // Score final na tela de vitória
  if (gameState == GAME_WIN) {
    int finalScore = DoomStickCGameplay::computeFinalScore();
    uint32_t totalSec = DoomStickCGameplay::score.totalTimeMs / 1000;
    frame.setTextColor(rgb(255, 220, 80), BLACK);
    frame.drawString(
      String(DoomStickCUI::SCORE_PREFIX) + String(finalScore),
      SCREEN_W / 2, 94, 2
    );
    frame.setTextColor(rgb(120, 255, 180), BLACK);
    frame.drawString(
      String(DoomStickCUI::KILLS_PREFIX) + String(DoomStickCGameplay::score.totalKills) +
      String("  ") + String(DoomStickCUI::TIME_PREFIX) + String(totalSec) + String("s"),
      SCREEN_W / 2, 110, 1
    );
  }

  frame.setTextColor(rgb(120, 255, 180), BLACK);
  frame.drawString(DoomStickCUI::RESTART_HINT, SCREEN_W / 2, 126, 1);
  frame.pushSprite(0, 0);
}

static void renderFrame() {
  if (gameState == GAME_LOADING)      { drawPeekSecurityLoadingScreen(); return; }
  if (gameState == GAME_INTRO)        { drawIntroScreen();               return; }
  if (gameState == GAME_LEVEL_START)  { drawLevelStartScreen();          return; }
  if (gameState == GAME_LEVEL_CLEAR)  { drawLevelClearScreen();          return; }
  if (gameState == GAME_DEAD)         { drawEndScreen(DoomStickCUI::DEAD_TITLE, DoomStickCUI::DEAD_SUBTITLE, RED); return; }
  if (gameState == GAME_WIN)          { drawEndScreen(DoomStickCUI::WIN_TITLE, DoomStickCUI::WIN_SUBTITLE, rgb(120, 255, 180)); return; }
  renderWorld();
  frame.pushSprite(0, 0);
}
...
static void handleInput(float dt) {
  ...
  if (gameState == GAME_INTRO) {
    // Power muda dificuldade
    if (isPowerDown() && !powerWasDown) {
      powerWasDown = true;
      using D = DoomStickCGameplay::Difficulty;
      int next = (static_cast<int>(DoomStickCGameplay::currentDifficulty) + 1) % 3;
      DoomStickCGameplay::currentDifficulty = static_cast<D>(next);
      playToneSafe(DoomStickCAudio::TONE_PICKUP, 60);
    }
    if (!isPowerDown()) powerWasDown = false;
